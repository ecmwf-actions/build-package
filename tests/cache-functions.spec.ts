...
describe("getCacheKey", () => {
    ...
    it("returns cache key if dependencies object is undefined", async () => {
        ...
        const cacheKeyStr = `v=${version}${cacheSuffix}::cmake=${
            testEnv.CMAKE_VERSION
        }::options=${buildOptions.join()}::${repo}=${sha}`;
        const cacheKeySha = crypto
            .createHash("sha1")
    ...
    it.each`
        ...
        ${emptyObject}
    `(
        "logs error if repository HEAD fetch fails ($error)",
        async ({ error }) => {
    ...
    it("invalidates the cache if suffix is supplied", async () => {
        ...
        let newCacheKeyStr = `v=${version}${testCacheSuffix}::cmake=${
            testEnv.CMAKE_VERSION
        }::options=${buildOptions.join()}::${repo}=${sha}`;
